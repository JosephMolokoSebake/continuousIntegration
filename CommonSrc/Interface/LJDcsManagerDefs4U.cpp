...
#include "LJDcsManagerDefs4U.h"
#include "Cst4U_Manager.h"
...
void DCSStatusClass::SetCurrentStatus(DCS_STATUS_TYPE Stat,Cst4U_Manager* pRX)
{
	String sStat="";
	CurrStat=Stat;
	switch (CurrStat)
...
void DCSStatusClass::TXRequest(TIdUDPServer* pServer,const DCS_STATUS_TYPE Req)
{
	if (pServer)
	{
		...
		{
			Cst4U_Request Msg;
			Msg.Set(JTime(true),nCst4SiteID,Req,0);
			BYTE aucBuf[1024];
...
void DCSStatusClass::ShutdownSystem()
{
	if (pOwner != NULL)
	{
		...
		if (nClient >= 0)
		{
			...
			Cst4U_Request Req(JTime(true),nCst4SiteID,CST4_REQUEST_SHUTDOWN,0);
			BYTE aucBuf[1024];
