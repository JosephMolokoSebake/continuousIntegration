...
void SymbolStruct::Draw(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer, const TColor BackColor)
{
	pDraw->SetCurrentLayer(Layer);
...
//         if (bSolidBack)
//            {
            pDraw->SetBrush(FrameBackColor,bsSolid);
            pDraw->SetPen(FrameColor,nFrameWidth,psSolid);
            pDraw->Rectangle(nx-nExtra,ny-nExtra,nx+Size.nW+nExtra,ny+Size.nH+nExtra,bSolidBack);
//            }
//         else
//            {
//            pDraw->SetPen(FrameColor,nFrameWidth,psSolid);
//            pDraw->MoveTo(nx-nExtra,ny-nExtra);
//            pDraw->LineTo(nx+nExtra+Size.nW,ny-nExtra);
//            pDraw->LineTo(nx+nExtra+Size.nW,ny+Size.nH+nExtra);
//            pDraw->LineTo(nx-nExtra,ny+Size.nH+nExtra);
//            pDraw->LineTo(nx-nExtra,ny-nExtra);
//            }
         }
      else
         {
         if (bSolidBack)
	         pDraw->FillRect(TRect(nx,ny,nx+Size.nW,ny+Size.nH),BackColor);
         }
      int nyL=ny;
      String s;
      int nTextHeight;
...
//         ::SetBkMode(DC,nOldMode);
   ...
   if ((bDispSymbol)&&(bDisplay))
      {
      pDraw->SetPen(SymbolColor,1);
      if (bSolidFill)
      	pDraw->SetBrush(SymbolColor);
      else
      	pDraw->SetBrush(BackColor);
      switch(Symbol)
         {
         case CIRCLE_SYM:
         	pDraw->Ellipse(PixPos.nx,PixPos.ny,nSymbolSize,nSymbolSize,bSolidFill);
            break;
         case SQUARE_SYM:
         	pDraw->Rectangle(PixPos.nx-nSymbolSize,PixPos.ny-nSymbolSize,PixPos.nx+nSymbolSize,PixPos.ny+nSymbolSize,bSolidFill);
            break;
         case DIAMOND_SYM:
            pDraw->MoveTo(PixPos.nx,PixPos.ny-nSymbolSize);
            pDraw->LineTo(PixPos.nx+nSymbolSize,PixPos.ny);
            pDraw->LineTo(PixPos.nx,PixPos.ny+nSymbolSize);
            pDraw->LineTo(PixPos.nx-nSymbolSize,PixPos.ny);
            pDraw->LineTo(PixPos.nx,PixPos.ny-nSymbolSize);
            break;
         case CROSS_SYM:
            pDraw->MoveTo(PixPos.nx,PixPos.ny-nSymbolSize);
            pDraw->LineTo(PixPos.nx,PixPos.ny+nSymbolSize);
            pDraw->MoveTo(PixPos.nx-nSymbolSize,PixPos.ny);
            pDraw->LineTo(PixPos.nx+nSymbolSize,PixPos.ny);
            break;
         default: ;
         }
...
void SymbolStruct::DrawRect(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer)
{
   if (PixPos.nx>=0)
      {
      pDraw->SetCurrentLayer(Layer);
      pDraw->MoveTo(Bounds.Left,Bounds.Top);
      pDraw->LineTo(Bounds.Right,Bounds.Top);
      pDraw->LineTo(Bounds.Right,Bounds.Bottom);
   	pDraw->LineTo(Bounds.Left,Bounds.Bottom);
      pDraw->LineTo(Bounds.Left,Bounds.Top);
      }
}
...
void SymbolStruct::Store(JIniFile* pFil, const String sName)
{
   ...
   pFil->Write(L"Symbol Type",Symbol);
   ...
   pFil->Write(L"Alignment",TextAlign);
...
//   NavSymbolForm->DisableCircle();	//Only for Direct2D
...
void SymbolStruct::CalcSize(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer, const bool bStandAlone)
{
   if (bDispText)
      {
      pDraw->SetCurrentLayer(Layer);
      ...
      if (pText->Count>0)
         ...
         //Use dummy text to ensure that the true width and height will lways be given if
         //the font has changed, but the text stays the same
         TextSize.cx=pDraw->nTextWidth("#!");

         for (int i=0; i<pText->Count; i++)
            {
            String s=pText->Strings[i];
            TextSize.cx=pDraw->nTextWidth(s);
            TextSize.cy=pDraw->nTextHeight(s);
...
void LineStruct::Draw(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer, const TColor)
{
	...
	if ((SegPixList.nGetCount()>1)&&(bDisplay))
		...
		if (SegPixList[0]->bValid())
         ...
         pDraw->SetPen(Layer,Color,nWidth,psSolid);
			pDraw->DrawPolyLine(Layer,pPnt,SegPixList.nGetCount());
         delete[] pPnt;
         }
      }
   else
      ...
      if ((aPixPos[0].bValid())&&(bDisplay))
         ...
         for (int j=0; j<2; j++)
            ...
            if (abShowArrow[j])
               ...
               aPnt[2].x=dXa;
               aPnt[2].y=dYa;
		         pDraw->SetPen(Layer,Color,1,psSolid);
               pDraw->SetBrush(Layer,Color,bsSolid);
					pDraw->DrawPolygonRegion(Layer,aPnt,3);
         ...
         if (nWidth<=1)
      	...
      	pDraw->MoveTo(Layer,aPix[0].nx,aPix[0].ny);
         pDraw->LineTo(Layer,aPix[1].nx,aPix[1].ny);
         }
		}
}
...
{
   NavLineForm->Ed.Init();
   NavLineForm->Ed.nSetLL("Line Start Point",NavLineForm->Lat1Ed,NavLineForm->Long1Ed,aPos[0]);
   NavLineForm->Ed.nSetVal(NavLineForm->Head1Ed,anArrowSize[0],"Arrow 1 Size",1,99);
   NavLineForm->Head1UD->Position=anArrowSize[0];
   NavLineForm->Arrow1Chk->Checked=abShowArrow[0];
   NavLineForm->Ed.nSetLL("Line End Point",NavLineForm->Lat2Ed,NavLineForm->Long2Ed,aPos[1]);
   NavLineForm->Ed.nSetVal(NavLineForm->Head2Ed,anArrowSize[1],"Arrow 2 Size",1,99);
...
void ArcStruct::Draw(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer, const TColor BackColor)
{
   if (bDisplay)
      {
      pDraw->SetCurrentLayer(Layer);
      if (bFill)
      	pDraw->SetBrush(FillColor);
      else
      	pDraw->SetBrush(BackColor);
   	pDraw->SetPen(FrameColor,nFrameWidth);
      if (pPoly)
         {
         if (bFill)
         	{
//            pDraw->SetBrush(FillColor);
	         pDraw->DrawPolygonRegion(pPoly,nPoints);
            }
         else
	         pDraw->DrawPolyLine(pPoly,nPoints);
         if (ArcType==FOOTPRINT_ARC)
            pDraw->Ellipse(PixPos.nx,PixPos.ny,4,4,false);
         }
      else if (ArcType==CIRCLE_ARC)
         {
         pDraw->Ellipse(PixPos.nx,PixPos.ny,nCirclePix,nCirclePix,bFill);
         if (bShowDot)
            pDraw->Ellipse(PixPos.nx,PixPos.ny,2,2,false);
         }
      }
}
...
{
   NavArcForm->Ed.Init();
   NavArcForm->Ed.nSetLL("Firing Position",NavArcForm->LatEd,NavArcForm->LongEd,Pos);
   NavArcForm->Ed.nSetVal(NavArcForm->FRaEd,nFwdRa,"");
   NavArcForm->Ed.SetLastLimits("Forward Range",10,99999);
   NavArcForm->Ed.nSetVal(NavArcForm->ARaEd,nAftRa,"");
   NavArcForm->Ed.SetLastLimits("Aft Range",10,99999);
   NavArcForm->Ed.nSetVal(NavArcForm->BeEd,dCenterBe,1,"");
   NavArcForm->Ed.SetLastLimits("Centre Bearing",0.0,359.9);
   NavArcForm->Ed.nSetVal(NavArcForm->ArcEd,dArcWidth,1,"");
   NavArcForm->Ed.SetLastLimits("Arc Width",0.0,359.9);
   NavArcForm->DescrEd->Text=sDescr;
   NavArcForm->Ed.nSetVal(NavArcForm->CircleEd,nCircleDia,"");
   NavArcForm->Ed.SetLastLimits("Circle Diameter",10,99999);
   NavArcForm->CircleDotChk->Checked=bShowDot;
   NavArcForm->FramePan->Color=FrameColor;
   NavArcForm->Ed.nSetVal(NavArcForm->WidthEd,nFrameWidth,"");
   NavArcForm->Ed.SetLastLimits("Arc Line Width",1,99);
...
void ManVectStruct::Draw(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer, const int nID)
{
   if (bDisplay)
      {
      pDraw->SetCurrentLayer(Layer);
      pDraw->SetPen(Color,nWidth,psSolid);
      pDraw->MoveTo(aPixPos[0].nx,aPixPos[0].ny);
      pDraw->LineTo(aPixPos[1].nx,aPixPos[1].ny);
      pDraw->SetFont(pFont);
//      ::SetTextAlign(DC,TA_TOP|TA_LEFT);
//		TCHAR szSt[8];
		String s=String(nID);
		SIZE Size;
      Size.cx=pDraw->nTextWidth(s);
      Size.cy=pDraw->nTextHeight(s);
      int nx=IDPixPos.nx;
      int ny=IDPixPos.ny;
      pDraw->SetPen(Color,1);
		pDraw->SetBrush(IDColor);
		int nR=sqrt(Size.cx*Size.cx+Size.cy*Size.cy*1.0)*0.6;
		pDraw->Ellipse(nx,ny,nR,nR,true);
      pDraw->TextOutW(IDPixPos.nx-Size.cx/2,IDPixPos.ny-Size.cy/2,s);
		}
}
...
bool NavAidClass::bRead(const String sName, LatLong& ZoomCenter, SizeD& ZoomSize)
{
   ...
   NavReadForm->Ed.SetLLRO(NavReadForm->NLatEd,NavReadForm->NLongEd,NewCenter);
...
void NavAidClass::DrawAll(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer, const TColor BackColor)
{
   for (int i=0; i<ArcList.nGetCount(); i++)
      ArcList[i]->Draw(pDraw,Layer,BackColor);
   DrawRef(pDraw,Layer,BackColor);
   for (int i=0; i<VectList.nGetCount(); i++)
      VectList[i]->Draw(pDraw,Layer,i+1);
   for (int i=0; i<LineList.nGetCount(); i++)
      if (i!=nLineDragNum)
         LineList[i]->Draw(pDraw,Layer,BackColor);
   for (int i=0; i<SymbolList.nGetCount(); i++)
      {
      if (SymbolList[i]->bRef)
         nRefSymbol=i;
      if (i!=nSymDragNum)
         SymbolList[i]->Draw(pDraw,Layer,BackColor);
      }
}

void NavAidClass::DrawArcs(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer, const TColor BackColor)
{
   for (int i=0; i<ArcList.nGetCount(); i++)
      ArcList[i]->Draw(pDraw,Layer,BackColor);
}

void NavAidClass::DrawVectors(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer)
{
   for (int i=0; i<VectList.nGetCount(); i++)
      VectList[i]->Draw(pDraw,Layer,i+1);
}

void NavAidClass::DrawLines(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer, const TColor BackColor)
{
   for (int i=0; i<LineList.nGetCount(); i++)
      if (i!=nLineDragNum)
         LineList[i]->Draw(pDraw,Layer,BackColor);
...
void NavAidClass::DrawSymbols(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer, const TColor BackColor)
{
   for (int i=0; i<SymbolList.nGetCount(); i++)
      {
      if (SymbolList[i]->bRef)
...
void NavAidClass::CalcSymbolSize(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer)
{
   for (int i=0; i<SymbolList.nGetCount(); i++)
      SymbolList[i]->CalcSize(pDraw,Layer);
}
...
TRect NavAidClass::GetCurrentSymbolRect(const int nx, const int ny)
{
   SymbolList[nCurrentSymbol]->SetPos(Pixel(nx,ny));
   return SymbolList[nCurrentSymbol]->GetBounds();
}

void NavAidClass::DrawSymbolRect(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer, const Pixel& Pix, const LatLong& LL)
{
   SymbolList[nCurrentSymbol]->SetPos(LL);
   SymbolList[nCurrentSymbol]->SetPos(Pix);
   SymbolList[nCurrentSymbol]->DrawRect(pDraw,Layer);
}
...
void NavAidClass::TransformPos(TransGraph* pG, const bool bCalcSize)
{
   ...
   if (bCalcSize)
      ...
      for (int nS=0; nS<nSymbols(); nS++)
         {
         SymbolStruct* pS=pGetSymbol(nS);
         pS->CalcSize(pG,DWG_LAYER_BACK_BMP,true);
         }
      }
...
void NavAidClass::DrawRef(D2DDraw* pDraw, const DWG_LAYER_TYPE Layer, const TColor BackColor)
{
   if (bRefSet())
      RefSymbol.Draw(pDraw,Layer,BackColor);
}
